...
public class PersonCollection implements PersonCollectionI {
    ...
    private int size;
    private int changesToCollection = 0;
    ...
    public Iterator<Person> iterator() {
        return new Iterator<Person>() {
            int position = 0;
            Person[] innnerPersons = persons;
            final int changeId = changesToCollection;

            @Override
            public boolean hasNext() {
                if (position >= innnerPersons.length || innnerPersons[position] == null){
                    return false;
                }
                return true;
            }
            ...
            @Override
            public Person next() {
                if (changeId != changesToCollection) throw new ConcurrentModificationException("Cant change collection while iterating");

                return innnerPersons[position++];
            }
        };
    }
