...
public class HashSetChaining {
	...
	public boolean add(Object x) {
		if (((currentSize+1) / (buckets.length * 1.0)) > 0.75){
			System.out.println(currentSize+1 / (buckets.length * 1.0));
			rehash();
		}
		int h = hashValue(x);
		...
		Node current = buckets[h];
		boolean found = false;
		while (!found && current != null) { //FIXME: Could this be replaced by contains method?
			if (current.data.equals(x)) {
				found = true;
				// Already in the set
			} else {
